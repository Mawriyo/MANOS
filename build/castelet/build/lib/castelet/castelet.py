...
from PyQt5.QtWidgets import QMainWindow, QAction, QMenu, QLabel
import rclpy
...
class Castlet(QMainWindow):
    def __init__(self, parent=None):
        ...
        # self.castelet_ui.LeftHand_GridLayout.addChildLayout
        ...
        self.subscriptions = {
            ...
            "topic": self.node.create_subscription(
                ListString, "/MANOS/ServiceDetector", self.getBindings, self.qos_profile
            ),
    ...
    def addLabelsToGridLayout(self):
        for i in range(5):
            label = QLabel(f"LeftHand_F{i+1}_Label")
            label.setObjectName(f"LeftHand_F{i+1}_Label")
            label.setStyleSheet(
                        """
                        color: #2c3e50;
            ...
            self.castelet_ui.LeftHand_GridLayout.addWidget(label, i, 0)
